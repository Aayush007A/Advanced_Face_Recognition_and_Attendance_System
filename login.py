...
class Login:
    def __init__(self,root):
        ...
        # Background Image
        img = Image.open(r'college_images\321138.jpg')
    ...
    def login(self):
        if (self.txtuser.get()=="" or self.txtpwd.get()==""):
            ...
            conn = mysql.connector.connect(username='Ronaldo7', password='a7b7e777',host='127.0.0.1',
                                           database='face_recognition_system')
            mycursor = conn.cursor()
            mycursor.execute("select * from register_user where Email=%s and Password=%s",(
                self.txtuser.get(),
                self.txtpwd.get()
            ))
            ...
            if row==None:
                ...
                open_min=messagebox.askyesno("YesNo","Access only Admin", parent=self.root)
                if open_min>0:
                    self.new_window=Toplevel(self.root)
    ...
    def reset_pass(self):
        if self.var_ssq.get()=="Select":
            ...
            conn = mysql.connector.connect(username='Ronaldo7', password='a7b7e777',host='127.0.0.1',
                                           database='face_recognition_system')
            mycursor = conn.cursor()
            query=("select * from register_user where Email=%s and Security_Question=%s and Security_Answer=%s")
            ...
            if row==None:
                ...
                query=("update register_user set Password=%s where Email=%s")
